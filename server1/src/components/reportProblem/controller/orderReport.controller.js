...
const reportOrderProblem = async(req, res, next) => {
    try{
        if(!req.user){
            throw new Error('Please signin to report order problem!')
        }
        ...
        //send mail to seller
        const subjectSeller = 'Order Problem Reported'
        const messageSeller = `A customer of your store reported a problem in order delivered, with following problem statement and description.
        <br><strong>Subject: ${orderProblem.subject}</strong><br><strong>Description: ${orderProblem.description}</strong><br>
        <br><strong>Buyer ID: </strong>${orderProblem.buyerID}
        <br><strong>Buyer Email: </strong>${orderProblem.email}
        <br><strong>Problem ID: </strong>${orderProblem._id}
        <br><strong>Order ID: </strong>${orderProblem.orderID}
        <br>Thank you for your time.`        
        notification.sendNotificationMail(seller.email,subjectSeller,messageSeller,seller.name)

        //send mail to buyer
        const subjectBuyer = 'Problem Reported Successfully!'
        const messageBuyer = `Your order compalint has been reported successfully. 
        <br><strong>Subject: ${orderProblem.subject}</strong><br><strong>Description: ${orderProblem.description}</strong><br>
        We will try to resolve it as soon as possible.<br> Thank you for your time.
        `
        notification.sendNotificationMail(buyer.email,subjectBuyer,messageBuyer,buyer.name)
...
const updateOrderProblemById = async(req, res, next) => {
    try{
        ...
        //send mail to buyer
        const subjectBuyer = 'Problem Resolved'
        const messageBuyer = `Your order compalint has been resolved successfully. 
        ...
        `
        notification.sendNotificationMail(problem.email,subjectBuyer,messageBuyer,"Respected Customer")
