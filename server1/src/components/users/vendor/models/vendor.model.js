		...
		birthday: {
			...
			validate(DOB) {
				...
				if (!DOB.match(regexDOB))
					throw new Error(
						"Please enter date in correct format e.g 1/12/2021"
					);
				...
				if (DOB.match(regexDOB)) {
					...
					if (dtCurrent.getFullYear() - dtDOB.getFullYear() < 18) {
						throw new Error(
							"Age be at least 18 years to register!"
						);
					}
				}
			},
		...
		CNIC: {
			...
			validate(str) {
				...
				if (!str.match(regExpCNIC)) {
					throw new Error(
						"Please enter CNIC in correct format e.g. 12345-1234567-1"
					);
				}
			},
		},
...
vendorSchema.statics.findByCredientials = async (email, password) => {
	...
	if (!user && !password && !email) {
		throw new Error(
			"Unable to login! Please enter valid email and password! "
		);
	}
	if (!user) {
