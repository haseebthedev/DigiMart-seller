...
const addPromotedProduct = async (req, res, next) => {
	try {
		...
		if (store.isApprovedPromotionTool == false) {
			throw new Error(
				"Sorry! You cannot promote your products. Contact admin to activate your promotoion tool!"
			);
		}
		...
		const isPromoteToSavedPromotionAudience =
			req.body.isPromoteToSavedPromotionAudience;
		const productCategory = req.body.productCategory;
		if (isPromoteToSavedPromotionAudience) {
			const promotionAudience = await PromotionAudience.findOne({
				productCategory: { $in: productCategory },
			});
			if (!promotionAudience) {
				throw new Error(
					"Sorry! No promotion audience found of this product category!"
				);
			}
			...
			req.body.promotionAudienceCategory =
				promotionAudience.audienceInterestCategory;
			req.body.promotionAudiencePromotionSource =
				promotionAudience.promotionSource;
		...
		const importedAudiencePromotionSource =
			req.body.importedAudiencePromotionSource;
		//SMS
		if (importedAudiencePromotionSource == "SMS") {
			importedAudienceData.forEach(async (user) => {
				if (user.number) sendSMS.message(user.number, messageBody);
			});
		}
			...
			importedAudienceData.forEach((user) => {
				if (user.email)
					notification.sendNotificationMail(
						...
						subject,
						messageBody,
						user.name != null ? user.name : "Respeced Sir/Madam"
					);
			});
		}
		//BOTH
		...
		if (isPromoteToAllBuyers) {
			let storeBuyersIds = await Order.find(
				{ storeId: storeId },
				{
					_id: 0,
					buyerId: 1,
			...
			if (buyerPromotionSource == "SMS") {
				const storeBuyers = await Buyer.find(
					{ _id: { $in: storeBuyersIds } },
					{
						_id: 0,
						phoneNumber: 1,
					}
		...
		if (req.body.promotion_date) {
			...
			scheduleMessage =
				"On following date and time " + req.body.promotion_date;
		}
