...
const loginBuyer = async (req, res, next) => {
	try {
		console.log("comming from server");
		const buyer = await Buyer.findByCredientials(
			req.body.email,
			req.body.password
		);
...
const UnSubscribeStoreByStoreId = async (req, res, next) => {
	try {
		const storeId = req.body.storeId;
		const user = req.user;
		user.subscribedStores = user.subscribedStores.filter(
			(subscribedStoreId) => {
...
const ViewSubscribedStores = async (req, res, next) => {
	try {
		...
		const subscribedStores = await Store.find({
			_id: { $in: subscribedStoresIds },
		});
		//send email here
		return res.status(200).json({
...
const getAllBuyersDetailsForSeller = async (req, res, next) => {
	try {
		const filters = {};
		const Buyers = await Buyer.find(filters).select({
			_id: 1,
			name: 1,
			email: 1,
		});
...
const searchBuyerByName = async (req, res, next) => {
	try {
		...
		const Buyers = await Buyer.find({ name }).select({
			_id: 1,
			name: 1,
			email: 1,
		});
