...
const ViewWithdrawalsRequests = () => {
	...
	const getAllDetails = async () => {
		await api
			.get("/seller/store/requested/withdraws", {
				headers: { Authorization: `Bearer ${token}` },
			})
			.then((res) => {
				// console.log(res);
				setDetails(res.data.data.requestedWithdrawals);
			})
			.catch((error) =>
				console.log(
	...
	return (
		<Grid container className={classes.root}>
			<Grid item xs={12} sm={12} md={12} component={Paper}>
				<MaterialTable
					title="Payment Withdraw Requests"
					data={details}
					columns={columns}
					options={{
...
export default ViewWithdrawalsRequests;
