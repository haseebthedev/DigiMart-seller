...
const loginSeller = async (req, res, next) => {
    try{
        const seller = await Seller.findByCredientials(req.body.email,req.body.password)
...
const deleteMyAccount = async(req, res, next) => {
    try{  
        ...
        if(req.store){
            ...
            const problemReportsOfStore = await ReportProblem.deleteMany({sellerID: seller._id})
            const orderReportsOfStore = await OrderProblemReport.deleteMany({storeID: store._id})
            await store.remove()
        }
    ...
    try{
        ...
        if(req.store){
            if(req.user.status == "Blocked"){
                throw new Error('Sorry! Your account is blocked and it cannot be activated.')
            }
            //activate account status
...
const resetPassword = async(req, res, next) => {
    try{
        ...
        const seller = await Seller.findOne({resetPasswordToken: req.params.token, resetPasswordExpires: {$gt: Date.now()}})
        if(!seller){
...
const editSellerById = async(req, res, next) => {
    try{
        ...
        //if status block then logout seller from all devices
        if(req.body.status == "Blocked" || req.body.status == "Deactivate"){
            //logout from all devices
            user.tokens = []
        }
        //Update all fields of store acc to status
        const store = await Store.findById(user.storeId)
        if(store){
            store.status = req.body.status
            if(req.body.status == "Blocked" || req.body.status == "Deactivate"){
