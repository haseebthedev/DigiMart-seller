...
superAdminSchema.statics.findByCredientials= async(email,password)=>{
...
superAdminSchema.methods.generateAuthToken=async function(){
    const user=this
    const token=jwt.sign({_id:user._id.toString()},process.env.JWT_SUPER_ADMIN_CODE,{
        expiresIn: process.env.JWT_SUPER_ADMIN_EXPIRES_IN,
    })
...
superAdminSchema.statics.getStorageDetails = async function() {
    const Size = await SuperAdmin.collection.stats({scale: 1024});
...
superAdminSchema.plugin(uniqueValidator, { message: '{PATH} already exists!' });
